    ...
    public class BusquedaController : ControllerPestanyaBase
    {
        ...
        private List<ElementoGnoss> Buscar(ResultadoModel pListaResultados)
        {
            ...
            if (metaBuscadorCN.DocumentacionDW != null)
            {
                ...
                mGestionDocumentacion.GestorTesauro = new GestionTesauro(tesauroCL.ObtenerTesauroDeProyecto(mControladorBase.UsuarioActual.ProyectoID), mLoggingService, mEntityContext);
            }
        ...
        public ActionResult Index()
        {
            ...
            {
                new SeguridadCL(mEntityContext, mLoggingService, mRedisCacheWrapper, mConfigService, mServicesUtilVirtuosoAndReplication).ObtenerConexionAfinidad(tokenAfinidad.ToString());
            }
            ...
            if (!mEsVistaMapa)
            {
                ...
                ViewBag.ListaMetas.Add(new KeyValuePair<string, string>("description", metaTagDescription));
            }
        ...
        {
            ExportacionBusquedaCL exporBusCL = new ExportacionBusquedaCL(mEntityContext, mLoggingService, mRedisCacheWrapper, mConfigService, mServicesUtilVirtuosoAndReplication);
        ...
        private string DevolverStringSiesFecha(string valorpropiedad)
        {
            ...
            try
            {
                ...
                {
                    double fechaValida = double.Parse(valorpropiedad);
                    DateTime fecha = DateTime.ParseExact(valorpropiedad, "yyyyMMddHHmmss", null);
        ...
        private DataSet ObtenerDatosExportacionComunidad(DataWrapperExportacionBusqueda pExporBusDW, Guid pProyectoID, Guid pOrgIDProyBusquedaExcelCom, Guid pExportacionID, List<ElementoGnoss> pListaDefinitiva, List<string> pListaItemsBusqueda, TipoBusqueda pTipoBusqueda, string pArgumentos)
        {
            ...
            GestionFacetas gestorFacetas = new GestionFacetas(dataWrapperFacetas, mLoggingService);
            facetaCL.Dispose();
            ...
            foreach (ElementoGnoss elemento in pListaDefinitiva)
            {
                ...
                if (elemento is Documento)
                {
                    ...
                    else
                    {
                        ...
                        documentoWeb = new DocumentoWeb(documento.FilaDocumento, documento.GestorDocumental, mLoggingService);
                    }
                    ...
                    foreach (ProyectoPestanyaBusquedaExportacionPropiedad filaPropExport in pExporBusDW.ListaProyectoPestanyaBusquedaExportacionPropiedad.Where(item => item.ExportacionID.Equals(pExportacionID)).OrderBy(item => item.Orden))
                    {
                        ...
                        else if (filaPropExport.Propiedad == TipoPropExportacion.Publicador)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    creador = new IdentidadCN(mEntityContext, mLoggingService, mConfigService, mServicesUtilVirtuosoAndReplication).ObtenerNombreDeIdentidad(documentoWeb.FilaDocumentoWebVinBR.IdentidadPublicacionID.Value);
                                }
                        ...
                        else if (filaPropExport.Propiedad == TipoPropExportacion.Editores)
                        {
                            ...
                            if (documentoWeb.GestorDocumental.DataWrapperDocumentacion.ListaDocumentoRolIdentidad.Any(doc => doc.DocumentoID.Equals(documentoWeb.Clave)))
                            {
                                ...
                                List<string> editoresConRepetidos = new IdentidadCN(mEntityContext, mLoggingService, mConfigService, mServicesUtilVirtuosoAndReplication).ObtenerNombresDePerfilesUOrganizacion(editoresID).Values.ToList();
                ...
                if (elemento is Identidad)
                {
                    ...
                    foreach (ProyectoPestanyaBusquedaExportacionPropiedad filaPropExport in pExporBusDW.ListaProyectoPestanyaBusquedaExportacionPropiedad.Where(item => item.ExportacionID.Equals(pExportacionID)).OrderBy(item => item.Orden))
                    {
                        if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.FechaAlta))
                        {
                            fila.Add(UtilCadenas.ObtenerTextoDeIdioma(identidad.FilaIdentidad.FechaAlta.ToString(), IdiomaUsuario, null));
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Nombre))
                        {
                            fila.Add(UtilCadenas.ObtenerTextoDeIdioma(identidad.Persona.Nombre, IdiomaUsuario, null));
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Apellidos))
                        {
                            fila.Add(UtilCadenas.ObtenerTextoDeIdioma(identidad.Persona.Apellidos, IdiomaUsuario, null));
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Email))
                        {
                            fila.Add(UtilCadenas.ObtenerTextoDeIdioma(identidad.Persona.Mail, IdiomaUsuario, null));
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.FechaNacimiento))
                        {
                            fila.Add(UtilCadenas.ObtenerTextoDeIdioma(identidad.Persona.Fecha.ToShortDateString(), IdiomaUsuario, null));
                        }
                        ...
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.NumeroVisitas))
                        {
                            if (dicContadoresIdentidad != null && dicContadoresIdentidad.Count > 0 && dicContadoresIdentidad.ContainsKey(identidad.Clave))
                            {
                                fila.Add(dicContadoresIdentidad[identidad.Clave][IdentidadAD.CONTADOR_NUMERO_VISITAS].ToString());
                            }
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.NumeroDescargas))
                        {
                            if (dicContadoresIdentidad != null && dicContadoresIdentidad.Count > 0 && dicContadoresIdentidad.ContainsKey(identidad.Clave))
                            {
                                fila.Add(dicContadoresIdentidad[identidad.Clave][IdentidadAD.CONTADOR_NUMERO_DESCARGAS].ToString());
                            }
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Pais) && identidad.Persona.PaisID != Guid.Empty)
                        {
                            if (paisDW == null)
                            {
                                paisDW = paisCL.ObtenerPaisesProvincias();
                            }

                            fila.Add(paisDW.ListaPais.Where(item => item.PaisID.Equals(identidad.Persona.PaisID)).FirstOrDefault().Nombre);
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Provincia))
                        {
                            fila.Add(identidad.Persona.Provincia);
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.Localidad))
                        {
                            fila.Add(identidad.Persona.Localidad);
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.RedesSociales))
                        {
                        ...
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.NumRecursosComentados))
                        {
                            ...
                            fila.Add(contadorComentarios.ToString());
                        }
                        else if (filaPropExport.Propiedad.ToLower().Equals(TipoPropExportacionPersonas.FechaUltimoAcceso))
                        {
                            DateTime? fechaUltimoAcceso = usuarioCN.ObtenerFechaUltimoAccesoDeUsuario(identidad.Persona.UsuarioID);
                            string fecha = "";
                        ...
                        else
                        {
                            ...
                            {//"DatoExtraID='" + datoExtraID + "' and IdentidadID='" + identidad.Clave + "'"
                                ...
                                {
                                    fila.Add(UtilCadenas.ObtenerTextoDeIdioma(filasDatoExtraVirIdent.First().Opcion, IdiomaUsuario, null));
                                }
                                else
                                {
                                    fila.Add("");
        ...
        private void CargarDataSetPresentacionFacetado(ref FacetadoDS pFacDSPresentacion, List<string> pListaGrafos, string pParametros, Guid pProyectoSeleccionado, List<string> pListaPropiedades, Dictionary<string, List<string>> pInformacionOntologias)
        {
            ...
            FacetaCL facetaCL = new FacetaCL(mEntityContext, mLoggingService, mRedisCacheWrapper, mConfigService, mServicesUtilVirtuosoAndReplication);
            FacetadoCN facCN = new FacetadoCN(UrlIntragnoss, ProyectoSeleccionado.ToString(), mEntityContext, mLoggingService, mConfigService, mVirtuosoAD, mServicesUtilVirtuosoAndReplication);
            ...
            foreach (string grafo in pListaGrafos)
            {
                ...
                while (listaPropiedadesTemp.Count > 0)
                {
                    ...
                    {
                        UtilidadesVirtuoso utilidadesVirtuoso = new UtilidadesVirtuoso(mLoggingService, mEntityContext, mConfigService, mRedisCacheWrapper, mEntityContextBASE, mVirtuosoAD, mServicesUtilVirtuosoAndReplication);
        ...
        private GestionTesauro CargarGestorTesauroProyectoActual()
        {
            ...
            GestionTesauro gestorTesauro = new GestionTesauro(tesCL.ObtenerTesauroDeProyecto(ProyectoSeleccionado.Clave), mLoggingService, mEntityContext);
                            ...
                            try
                            {
                                ...
                                if (jsonRequest)
                                {
                                    ...
                                    using (MemoryStream output = new MemoryStream())
                                    {
                                        ...
                                        resultadoModel = (ResultadoModel)bformatter.Deserialize(output);
                                    }
                        ...
                        try
                        {
                            ...
                            if (jsonRequest)
                            {
                                ...
                                using (MemoryStream output = new MemoryStream())
                                {
                                    ...
                                    BinaryFormatter bformatter = new BinaryFormatter();
                                    facetedModel = (FacetedModel)bformatter.Deserialize(output);
