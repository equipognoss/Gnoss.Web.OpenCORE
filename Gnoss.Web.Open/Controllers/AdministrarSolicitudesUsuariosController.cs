...
namespace Gnoss.Web.Controllers
{
    public class AdministrarSolicitudesUsuariosController : ControllerBaseWeb
    {
        ...
        public IActionResult aceptar_solicitud(Guid solicitud_id)
        {
            ...
            DataWrapperIdentidad dataWrapperIdentidad = identCN.ObtenerPerfilesDePersona(solicitudUsuario.PersonaID, false);
            Es.Riam.Gnoss.AD.EntityModel.Models.IdentidadDS.Identidad identidad = dataWrapperIdentidad.ListaIdentidad.Find(identidad => identidad.PerfilID == solicitudUsuario.PerfilID && identidad.ProyectoID == solicitud.ProyectoID);
            if (identidad != null && !identidad.FechaExpulsion.HasValue && !identidad.FechaBaja.HasValue)
            {
                return BadRequest($"El usuario ya cuenta con una identidad activa para el perfil con id {solicitudUsuario.PerfilID} que solicito la solicitud con id {solicitud_id}");
            }

            DataWrapperUsuario dataWrapperUsuario = new UsuarioAD(mLoggingService, mEntityContext, mConfigService, mServicesUtilVirtuosoAndReplication, mLoggerFactory.CreateLogger<UsuarioAD>(), mLoggerFactory).ObtenerUsuarioCompletoPorID(solicitudUsuario.UsuarioID);
            ...
            controladorDeSolicitudes.RegistrarUsuarioEnProyectoAutomatico(perfilUsuario, usuario, gestorUsuarios, gestorIdentidades);
            ...
            Guid identidadID = ObjetoIdentidadProy.Clave;
            Guid perfilID = ObjetoIdentidadProy.FilaIdentidad.PerfilID;
            ...
            controladorPersonas.ActualizarModeloBASE(gestorIdentidades.ListaIdentidades[identidadID], ProyectoSeleccionado.Clave, true, true, PrioridadBase.Alta, mAvailableServices);

            ControladorDocumentacion.ActualizarGnossLive(ProyectoSeleccionado.Clave, perfilID, Es.Riam.Gnoss.AD.Live.AccionLive.Agregado, (int)Es.Riam.Gnoss.AD.Live.TipoLive.Miembro, false, PrioridadLive.Alta, mAvailableServices);        
