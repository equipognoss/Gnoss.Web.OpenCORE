    ...
    public class AdministrarCategoriasController : ControllerBaseWeb
    {
        ...
        public ActionResult Guardar()
        {
            ...
            List<object> categoriasEliminadas = mEntityContext.ObtenerElementosEliminados(typeof(AD.EntityModel.Models.Tesauro.CategoriaTesauro));
                //(TesauroDS.CategoriaTesauroRow[])GestorTesauro.TesauroDS.CategoriaTesauro.Select("", "", DataViewRowState.Deleted);
        ...
        private ActionResult OrdenarCategoria_Event()
        {
            ...
            short ordenDestino = -1;

            if (guidCatSup != Guid.Empty)
            {
                if (GestorTesauro.ListaCategoriasTesauroPrimerNivel.ContainsKey(CategoriasSeleccionadasIDs[0]))
                {//Si las categorias seleccionadas son del primer nivel
                    ordenDestino = (short)(GestorTesauro.ListaCategoriasTesauro[guidCatSup].FilaCategoria.Orden);
                }
                else
                {//Si no son del primer nivel
                    ordenDestino = (short)GestorTesauro.ListaCategoriasTesauro[guidCatSup].FilaAgregacion.Orden;
                }
            }

            PaginaModel.PasosRealizados += ((short)AccionConCategorias.OrdenarCategoria).ToString() + "|_|" + ordenDestino.ToString();

            foreach (Guid guidCatSel in CategoriasSeleccionadasIDs)
            {
                PaginaModel.PasosRealizados += "|_|" + guidCatSel.ToString();
            }

            PaginaModel.PasosRealizados += "|,|";

            OrdenarCategoriasTesauro(ordenDestino, CategoriasSeleccionadasIDs);
        ...
        private void RestarOrdenCategoriasMover(short pOrdenOriginal, Guid pCatID, Guid pCatTesauroSupID)
        {
            ...
            if (!pCatTesauroSupID.Equals(Guid.Empty))
            {
                ...
                {
                    if (!CategoriasCompartidasIDs.Contains(catTesHermana.Clave) && pOrdenOriginal < catTesHermana.FilaAgregacion.Orden)
                    {
            ...
            else
            {
                ...
                {
                    if (!CategoriasCompartidasIDs.Contains(catTesRaiz.Clave) && pOrdenOriginal < catTesRaiz.FilaCategoria.Orden)
                    {
        ...
        private void MoverCategoriasTesauro(Guid pCategoriaPadre, List<Guid> pCategoriasAMover)
        {
            ...
            foreach (Guid guidCat in pCategoriasAMover)
            {
                ...
                else
                {
                    ...
                    RestarOrdenCategoriasMover(ordenOriginal, guidCat, clave);
                    ...
                    short nuevoOrden = ObtenerNuevoOrdenMover(pCategoriaPadre);

                    if (pCategoriaPadre != Guid.Empty)
                    {
                        GestorTesauro.ListaCategoriasTesauro[guidCat].FilaAgregacion.Orden = nuevoOrden;
                        GestorTesauro.ListaCategoriasTesauro[guidCat].FilaCategoria.Orden = nuevoOrden;
                    }
                    else
                    {
                        GestorTesauro.ListaCategoriasTesauro[guidCat].FilaCategoria.Orden = nuevoOrden;
        ...
        private void OrdenarCategoriasTesauro(int pOrden, List<Guid> pListaCategorias)
        {
            ...
            {
                sql = sql.Where(item => item.CategoriaSupDestinoID.Equals(categoriaPadre));
            }

            short cont = 0;

            AD.EntityModel.Models.Tesauro.CatTesauroCompartida[] catCompHermanas = sql.ToArray();
            foreach (AD.EntityModel.Models.Tesauro.CatTesauroCompartida filaTesCompartida in catCompHermanas)
            {
        ...
        /// <param name="ordenesAntiguos"></param>
        /// <param name="pOrdenCatSeleccionada"></param>
        /// <param name="pOrden"></param>
        /// <param name="pCont"></param>
        /// <returns></returns>
        private short obtenerOrdenOrdenar(bool pEstaSeleccionada, List<short> ordenesAntiguos, short pOrdenCatSeleccionada, int pOrden, short pCont)
        {
            ...
            else
            {
                ...
                pCont++;
                return (short)(pOrden + pCont + cambioOrden);
            }
        }
