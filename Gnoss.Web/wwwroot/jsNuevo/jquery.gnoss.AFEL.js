...
var commentsAnalitics = {
    config: function (type) {
        ...
        analiticsTriples.push(analitics.getTriple("http://www.w3.org/1999/02/22-rdf-syntax-ns#type", "<http://vocab.afel-project.eu/Comment>"));
...
var searchAnalitics = {
    config: function (type) {
        ...
        analiticsTriples = [];
        analiticsTriples.push(analitics.getTriple("actionType", type));
        analiticsTriples.push(analitics.getTriple("http://www.w3.org/1999/02/22-rdf-syntax-ns#type", "<http://vocab.afel-project.eu/UserActivity>"));
        analiticsTriples.push(analitics.getTriple("date", date));
    },
...
var analitics = {
    ...
    send: function (analiticsTriples) {
		...
		if(this.graph != null && this.graph != ""){
            ...
            if(this.isAcceptedEvent(analiticsTriples)){
                //$.post("http://ext200.know-center.tugraz.at/afeldatafetcher/afel_gnossdata_fetcher/dynamicpushdata/gnosspush.php", { rdf: listaTriples });
                $.post("https://ext200.know-center.tugraz.at/afelgnosspush/gnosspush.php", { rdf: listaTriples });
            }
		}
    },
    ...
    isAcceptedEvent: function (analiticsTriples) {
        var blockedEvents = ["playStart","playStudyChange","languageChange","audioStateChange","labelStateChange","answersDetailsStateChange","dragMap","zoomMap","playMapClic","studyMapClic","playEnd","launchChange","acceptRejectChange","challengeEnd",]
        
        var eventType = "";
        $.each(analiticsTriples, function (index, value) {
            if(value.indexOf("<http://gnoss.com/ontology.owl#actionType>") > -1){
                var tripleParts = value.split(" ");
                eventType = tripleParts[2];
                eventType = eventType.replace(".","");
                eventType = eventType.replace(new RegExp("\"", 'g'), "");
            }
        });
        
        if(blockedEvents.indexOf(eventType) != -1){
            return  false;
        }
        return true;
    }
