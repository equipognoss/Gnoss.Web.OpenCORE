...
function saveContent() {
    ...
    e.undoManager.transact(function() {
        ...
        // Decodificar contenido dentro de <pre><code></code></pre>
        /*content = content.replace(/<pre[^>]*><code[^>]*>([\s\S]*?)<\/code><\/pre>/gi, function(match, p1) {
            return '<pre><code>' + decodeEntities(p1) + '</code></pre>';
        });
        */
        // Decodificar contenido dentro de <pre><code></code></pre> manteniendo las clases
        content = content.replace(/<pre([^>]*)><code([^>]*)>([\s\S]*?)<\/code><\/pre>/gi, function(match, p1, p2, p3) {
            return '<pre' + p1 + '><code' + p2 + '>' + decodeEntities(p3) + '</code></pre>';
        });        
...
function decodeEntities(encodedString) {
    ...
    const decodedString = textArea.innerHTML;
...
!(function () {
	...
	(c.src =
		...
		(tryToBuildAceTimer = setInterval(() => {
			try {
				(aceEditor = ace.edit("tox-codeeditor-editor")).setTheme(
					"ace/theme/twilight"
				),
		...
		tinymce.PluginManager.add("codeeditor", function (e) {			
			return (
				e.ui.registry.addButton("codeeditor", {
					...
					onAction: function () {
						// Establecer el editor correcto. Si hay múltiples, fallaba ya que no detecta el focus al pulsar directamente en el botón de "codeEditor"
						tinymce.setActive(e);
						displayToxEditorModal();
						let o = html_beautify(
								e.dom.decode(e.getContent({ source_view: !0 }))
							),
							n = ace.createEditSession(o, "ace/mode/html");
						n.setUseWrapMode(t), aceEditor.setSession(n);
					},
				}),
				{
