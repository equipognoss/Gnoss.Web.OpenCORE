...
; (function ($) {
    ...
    $[plugin_name] = function (element, options) {
        ...
        plugin.getAllowedFormatsString = function () {
            ...
            const acceptedFiles = plugin.settings.acceptedFiles.split(",");
            switch (acceptedFiles) {
                ...
                case 'video':
                    ...
                    break;
                case 'owl':
                    formats = 'archivos de ontologías';
                    break;
                case 'xml':
                    formats = 'archivos xml';
                    break;                 
                case 'html':
                    formats = 'archivos html';
                    break;                  
                case 'css':
                    formats = 'archivos css';
                    break; 
                case 'js':
                    formats = 'archivos js';
                case 'xls':
                    formats = 'archivos excel';                    
                break;                       
                case 'xlsx':
                    formats = 'archivos excel';                    
                break;                                                                       
        ...
        plugin._validateFormat = function (file) {
            ...
            if (acceptedFiles === "owl" || acceptedFiles === ".owl" ) {
                fileAccepted = true;
            }            

            if (acceptedFiles === "css" || acceptedFiles === ".css" ) {
                fileAccepted = true;
            }  
            if (acceptedFiles === "xml" || acceptedFiles === ".xml" ) {
                fileAccepted = true;
            }               
            
            if (acceptedFiles === "js" || acceptedFiles === ".js" ) {
                fileAccepted = true;
            }               

            if (acceptedFiles === "html" || acceptedFiles === ".html" ) {
                fileAccepted = true;
            } 
            
            if (acceptedFiles === "xlsx" || acceptedFiles === ".xlsx" || acceptedFiles === "xls" || acceptedFiles === ".xls" ){
                fileAccepted = true;
            }
            
            // Check if there is more than one acceptedFile (array)
            /*if (acceptedFiles.includes(",")){
                //const acceptedFilesArrayItems = acceptedFiles.split(",");
                if (Array.isArray(acceptedFiles) && acceptedFiles.length) {
    ...
    $.fn[plugin_name] = function (options, ...args) {
        return this.each(function () {
            const node = this;
            // Eliminar "." adicionales y asignar el tipo correspondiente para cada GnossDrag and Drop
            //const acceptExtensions = this.accept.split(",").map( (extension) => extension.replace(".",""));
            const acceptExtensions = this.accept.split(",");
            this.accept = acceptExtensions.toString();
            // Asignarlo a options para cada dragAndDrop
            options.acceptedFiles = this.accept != undefined ? this.accept : "*";            
