...
(function () {
    var imageDialog = function (editor, dialogType) {
        ...
        return {
            //title: editor.lang.image[dialogType == 'imagesimple' ? 'title' : 'titleButton'],
            title: form.selectImg,
            minWidth: 420,
            minHeight: 360,
            onShow: function () {
                ...
                if (link) {
                    ...
                    // Fill out all fields.
                    if (dialogType == 'imagesimple')
                        this.setupContent(LINK, link);
                }

                if (element && element.getName() == 'img' && !element.data('cke-realelement')
					|| element && element.getName() == 'input' && element.getAttribute('type') == 'imagesimple') {
                ...
                $('#divCargCKEImg').css('display', 'none');
                var file = $('#fuCKEditor');
                file.css('display', '');
                var fileType = $('#divContFileTypeUpImgCKE');
            ...
            onOk: function () {
                ...
                if (this.imageEditMode) {
                    ...
                    if (dialogType == 'imagesimple' && imgTagName == 'input' && confirm(editor.lang.image.button2Img)) {
                        // Replace INPUT-> IMG
                        imgTagName = 'img';
                        this.imageElement = editor.document.createElement('img');
                    ...
                    else if (dialogType != 'imagesimple' && imgTagName == 'img' && confirm(editor.lang.image.img2Button)) {
                        // Replace IMG -> INPUT
                        imgTagName = 'input';
            ...
            onLoad: function () {
                ...
                div.id = 'divContFileUpImgCKE';
                div.setAttribute('style', 'margin:30px;');
                ElemVisible('.cke_dialog_body').appendChild(div);
                RecagarFileUpload();

                var div_Tipo_Archivos = document.createElement("div");
                div_Tipo_Archivos.id = 'divContFileTypeUpImgCKE';
                div_Tipo_Archivos.innerHTML = form.validExtensions;
                div_Tipo_Archivos.setAttribute('style', 'margin:30px;');
                ElemVisible('.cke_dialog_body').appendChild(div_Tipo_Archivos);

                var testframe = document.createElement("iframe");
                testframe.name = "frmUploadWorker";
                document.body.appendChild(testframe);

                urlCKEImg = $(ElemVisible('.cke_dialog_ui_hbox_first input'));
                botonCKEAceptar = ElemVisible('.cke_dialog_ui_hbox.cke_dialog_footer_buttons a.cke_dialog_ui_button_ok');



                $(ElemVisible('.cke_dialog_tabs')).css('display', 'none');
                $(ElemVisible('.cke_dialog_contents')).css('display', 'none');
            },
            onHide: function () {
                if (this.preview)
									...
									[
										{
										    ...
										    required: true,
										    onChange: function () {
										        //Fernando : imagenes con espacios o con caracteres estraños
										        var dialog = this.getDialog(),
													newUrl = urlDecode(this.getValue());
										    ...
										    commit: function (type, element) {
										        if (type == IMAGE && (this.getValue() || this.isChanged())) {
										            //Fernando : imagenes con espacios o con caracteres estraños
										            var newUrl = urlDecode(this.getValue());
										            element.data('cke-saved-src', newUrl);
    ...
    CKEDITOR.dialog.add('imagesimple', function (editor) {
        return imageDialog(editor, 'imagesimple');
    });
...
function RecagarFileUpload() {
    ...
    var onchange = "if(this.value.toLowerCase().endsWith('.png') || this.value.toLowerCase().endsWith('.jpg') || this.value.toLowerCase().endsWith('.jpeg') || this.value.toLowerCase().endsWith('.gif')){";
    onchange += "$(\'#divCargCKEImg\').css(\'display\', \'\');$(\'#fuCKEditor\').css(\'display\', \'none\');$(\'#divContFileTypeUpImgCKE\').css(\'display\', \'none\');this.form.submit();";
    onchange += "}else{";
    onchange += "$(\'#divCargCKEImg\').css(\'display\', \'none\');$(\'#fuCKEditor\').css(\'display\', \'\');$(\'#divContFileTypeUpImgCKE\').css(\'display\', \'\');$(\'#divContFileTypeUpImgCKE\').css(\'color\', \'red\');}";
    var html = '<form enctype="multipart/form-data" method="post" target="frmUploadWorker" action="' + urlbase + '/ConectorCKEditor.aspx" id="frmUpload"><input type="file" accept="image/*" id="fuCKEditor" name="fuCKEditor" onchange="' + onchange + '" ' + estilos + ' /><div id="divCargCKEImg" style="display:none;min-width:220px;"><img src="' + urlbase + '/img/espera.gif" style="vertical-align: sub; margin-right: 10px;"><label>' + form.cargando + '</label></div></form>';

    $('#divContFileUpImgCKE').html(html);
}
