...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	function onChangeSrc() {
		...
		if ( value !== widget.data.src ) {
			...
			preLoader( value, function( image, width, height ) {
				...
				// Obviar la anchura de la imagen para evitar imágenes deformadas (multidispositivos -> Imágenes grandes)
	...
	return {
		...
		onLoad: function() {
			...
			$('.cke_dialog_image_url').css('display', 'none');
		},
		onShow: function() {
			...
			//PAra que cuando se modifique la imagen vaya a la parte de edición
			var valorCampoUrl = widget.data.src;
			if(valorCampoUrl != 'undefined' && valorCampoUrl != '')
			{
				$('.cke_dialog_tab')[1].click();
			}
				...
				elements: [
					{
						...
						style: 'height:40px',
						onChange: function (){$('.cke_dialog_ui_fileButton .cke_dialog_ui_button').trigger( "click" );
